...
class ScansControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create scan" do
    assert_difference('Scan.count') do
      post scans_url, params: { scan: { name: @scan.name, result: @scan.result, time: @scan.time } }
    end
  ...
  test "should update scan" do
    patch scan_url(@scan), params: { scan: { name: @scan.name, result: @scan.result, time: @scan.time } }
